        ...
        process.nextTick(function () {
          ...
          http.get(apiUrl, function(res) {
            res.on('data', function(chunck){
              ...
              Steam.login(player.id, player.username, player.avatar, function(err, user) {
                if (err) {
                  return done(err);
                }
                console.log(done);
      ...
      if (uid !== null) {
        ...
        User.create(username, undefined, undefined, function(err, uid) {
          if (err !== null) {
            ...
            // Save their avatar
            var photoUrl = avatar;
            // photoUrl = path.dirname(photoUrl) + '/' + path.basename(photoUrl, path.extname(photoUrl)) + path.extname(photoUrl);
